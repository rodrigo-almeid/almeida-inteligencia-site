...
async def detectar_intencao(api_key: str, texto: str, config=None) -> str:
    ...
    prompt = (
        ...
        "- financeiro_consulta: o usuário PERGUNTA sobre seus gastos, contas ou quer um resumo. "
        "Palavras-chave: quanto, total, resumo, extrato, saldo, como estão, minhas contas, qual conta, que conta, quais contas, vence, vencimento, vencendo, pendente.\n"
        "  Exemplos: 'quanto gastei esse mês?', 'como estão minhas contas?', 'qual meu saldo?', "
        "'qual conta dia 31/07?', 'que contas vencem amanhã?', 'tem conta vencendo hoje?', "
        "'quais contas estão pendentes?', 'tem alguma conta pra pagar?', 'qual conta vence essa semana?'\n\n"
